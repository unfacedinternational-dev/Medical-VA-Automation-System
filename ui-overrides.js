...
(function(){
  ...
  function addNote(existing){
    ensureNotes();const n=existing||{id:'N-'+Date.now(),type:'joy',text:'',patient:''};
    ...
    if(n.patient)$('#nPatient').value=n.patient;$('#nText').value=n.text||'';
  }
  function noteOptions(noteId){const n=db.notes.find(x=>x.id===noteId);if(!n)return;modal('Note Options',`<p class="option-note"><b>${esc(noteTypes[n.type]?.label||'Note')}</b></p><p>${esc(n.text)}</p><p><b>Patient / record:</b> ${esc(n.patient||'General note')}</p>`,()=>{},'Close');const foot=$('#modalRoot .modal-foot');if(foot){const close=foot.querySelector('[data-close]');const edit=document.createElement('button');edit.className='btn primary';edit.textContent='Edit';edit.onclick=()=>{closeModal();addNote(n)};const del=document.createElement('button');del.className='btn danger';del.textContent='Delete';del.onclick=()=>{closeModal();deleteNote(noteId)};foot.insertBefore(del,close);foot.insertBefore(edit,close)}}
  ...
  function recordOptions(kind,recordId){const collection=db[kind];if(!Array.isArray(collection))return;const item=collection.find(x=>x.id===recordId);if(!item)return;const label={patients:'Patient',appointments:'Appointment',tasks:'Task',followups:'Follow-up',billing:'Billing record'}[kind]||'Record';const title=item.name||item.title||item.patient||item.claim||item.id||label;modal(`${label} Options`,`<p><b>${esc(title)}</b></p><p>Choose what to do with this record.</p>`,()=>{},'Close');const foot=$('#modalRoot .modal-foot');if(foot){const close=foot.querySelector('[data-close]');const edit=document.createElement('button');edit.className='btn primary';edit.textContent='Edit';edit.onclick=()=>{closeModal();editRecord(kind,recordId)};const del=document.createElement('button');del.className='btn danger';del.textContent='Delete';del.onclick=()=>{closeModal();deleteRecord(kind,recordId)};foot.insertBefore(del,close);foot.insertBefore(edit,close)}}
  ...
  views.notes=notes;
  const legacyNav=document.querySelector('[data-view="messages"]');if(legacyNav){legacyNav.dataset.view='notes';legacyNav.innerHTML='◇ <span>NOTES</span>'}
  ...
  // Replace the old notification destination so the user-facing area is NOTES, not Messages.
  document.addEventListener('click',e=>{
    const notification=e.target.closest('#notificationBtn');
    if(notification){e.preventDefault();e.stopImmediatePropagation();go('notes');return}
